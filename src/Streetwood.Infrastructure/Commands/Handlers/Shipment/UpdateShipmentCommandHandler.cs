...
namespace Streetwood.Infrastructure.Commands.Handlers.Shipment
{
    public class UpdateShipmentCommandHandler : IRequestHandler<UpdateShipmentCommandModel, Unit>
    {
        ...
        {
            await service.UpdateAsync(request.Id, request.Name, request.NameEng, request.Description, request.DescriptionEng, request.IsActive, request.Price, request.Type);
            return Unit.Value;
        }
    }
}
